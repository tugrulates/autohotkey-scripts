...
import { type Git, git as gitRepo } from "@tugrulates/internal/git";
  ...
  result.releases = {
    async list(options) {
      ...
      return response.data
        ...
        .filter((release) =>
          options?.isDraft === undefined || release.isDraft === options.isDraft
        );
    },
...
function pullRequest(
  ...
  repo: Repository,
  data: Partial<
    | GetResponseDataTypeFromEndpointMethod<typeof api.pulls.get>
    | GetResponseDataTypeFromEndpointMethod<typeof api.pulls.list>[0]
  >,
): PullRequest {
...
function release(
  ...
  repo: Repository,
  data: Partial<
    GetResponseDataTypeFromEndpointMethod<typeof api.repos.getRelease>
  >,
): Release {
  ...
  return {
    ...
    async assets() {
      ...
      return response.data.map((x) => releaseAsset(api, this, x));
    },
    async upload(file) {
      ...
      return releaseAsset(api, this, response.data);
    },
  };
}
...
function releaseAsset(
  ...
  release: Release,
  data: Partial<
    GetResponseDataTypeFromEndpointMethod<typeof api.repos.getReleaseAsset>
  >,
): ReleaseAsset {
